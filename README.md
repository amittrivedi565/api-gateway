...
to integrate your authentication service with the gateway, ensure route `auth` with `POST` method exist with specified url of `/aut/token` in your auth service if not configure them according to this.
```imp
     - name: auth
      host: localhost
      port: 5030
